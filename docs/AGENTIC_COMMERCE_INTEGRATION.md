...
This document provides instructions for integrating the Agentic Commerce Model Context Protocol (MCP) server into your AI orchestration environment.

## 1. Architectural Overview
The Agentic Commerce MCP Server provides a bridge between AI agents and the Commercetools microservice ecosystem. It utilizes the **SSE (Server-Sent Events)** transport to maintain a stable connection within the Docker environment.
...
4.  **Activate Tools**: Ensure the `agentic-commerce-mcp` toggle is enabled in the right-hand menu.
...
2.  **Register Server**: Add the Agentic Commerce bridge to the `mcpServers` section:
    ```json
    "mcpServers": {
      "agentic-commerce-mcp": {
        "url": "http://localhost:8087/sse"
...
While local hosts (LM Studio, Claude Desktop) connect via `localhost`, you can expose the Agentic Commerce bridge to remote cloud LLMs using a secure tunnel like **ngrok**:
